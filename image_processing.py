...
def process_images(folder_path): # runs overlay, find_holdinfo on each image, then returns dictionary with extracted information
    images_info = {}
    try:
...
## GRID OVERLAY ##
def overlay_folder(folder_path): # runs overlay on every image in a folder
    files_info = []
    try:
        for entry in os.listdir(folder_path): #open folder
            full_path = os.path.join(folder_path, entry) #get path of photo
...
def overlay(image_path): #runs detect-red_square, then overlays grid (lines 0.05m apart) and saves as image_path+"_grid"
    ...
    if not os.path.isfile(savedimage):
        ovrlay = img.copy() #make a copy of the image to overlay

        for cx in range(0, w, grid_px_x):
            cv2.line(ovrlay, (cx, 0), (cx, h), (0, 0, 255), 2) #add horizontal lines

        for cy in range(0, h, grid_px_y):
            cv2.line(ovrlay, (0, cy), (w, cy), (0, 0, 255), 2) #add vertical lines

        output = cv2.addWeighted(ovrlay, 0.8, img, 0.2, 0)
...
def find_holdinfo(image_path, ppmx, ppmy): # runs detect boxes, then converts pixels to meters
    x, y = "x", "y"
...
def detect_red_square(image): #detects the 0.2m x 0.2m square in image
...
def detect_boxes(image_path): #detects black boxes (holds) on image of climb. returns list of box positions and coords of origin
    ...
    for contour in contours: #for each contour
        ...
        if area > 750: #if big enough, it's a hold
            w //= 2; h //=2 #convert w and h to size (half of width/height)
            cv2.circle(img, center=(x+w,y+h), radius= 10,  color=(counter, 255, 0), thickness=-1) #debugging: draw circle for visualization
            box_positions.append({'x': x+w, 'y': dimensions[0]-(y+h), 'size_x': w, 'size_y': h}) # add hold dictionary to list and continue
        elif area > 70 and origin == -1: #if area wasn't big enough for a hold, but isn't small enough to be noise, it's the origin
